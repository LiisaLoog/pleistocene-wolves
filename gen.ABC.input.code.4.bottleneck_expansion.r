...
for(n in 1:7){
	...
	output <- as.matrix(read.table(gzfile(name), sep = "\t", skip = 26))
    #    output <- as.matrix(read.table(name, sep = "\t", skip = 12))
...
################################################################################################
	...
	x.bottel <- output[,8]
	exp.start <- output[,9]
	exp.interval <- output[,10]
	nsims <- length(sim.nr)

	abc.out.bottleneck.expansion <- cbind(log10(mig.rate), log10(Ka), log10(Kb), log10(Kc),x.bottel, exp.start, exp.interval, output[,11:length(output[1,])])

    dir.create(paste("ABC",n,sep = ""))
    setwd(paste("ABC",n,sep = ""))
    write.table(abc.out.bottleneck.expansion, file = paste("abc.out_",n,"bottelneck_expansion.", "1.0.txt", sep = "_"), sep = " ", col.names = c("log10(mig.rate)", "log10(Ka)","log10(Kb)","log10(Kc)", "x.bottel","exp.start","exp.interval","tEUR-EUR", "tEUR-ME", "tEUR-NEA","tME-NEA", "tNEA-NEA","tEUR-AM","tME-AM","tNEA-AM","tAM-AM"), row.names = F, quote = F)
    setwd("..")
}
