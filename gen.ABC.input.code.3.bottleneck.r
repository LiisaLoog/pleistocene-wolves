...
setwd("../3_Bottleneck/") # Provide a full path
...
output <- as.matrix(read.table(gzfile("a2.txt.gz"), sep = "\t", skip = 19))
#output <- as.matrix(read.table("a2.txt", sep = "\t", skip = 18))
...
abc.out.bottleneck <- cbind(log10(mig.rate), log10(Ka), log10(Kb),log10(Kc), output[,8:length(output[1,])])

dir.create("ABC")
setwd("ABC")
write.table(abc.out.bottleneck, file = paste("abc.out_bottleneck","1.0.txt", sep = "_"), sep = " ", col.names = c("log10(mig.rate)", "log10(Ka)", "log10(Kb)","log10(Kc)","tEUR-EUR", "tEUR-ME", "tEUR-NEA","tME-NEA", "tNEA-NEA","tEUR-AM","tME-AM","tNEA-AM","tAM-AM"), row.names = F, quote = F)
setwd("..")
